...
**Issue #9** (IAP: individual tour purchase) is the recommended next ticket now that #8 is closed end-to-end. The pure-logic shape of #9 (Codable purchase records → backend client → owned-products merge) mirrors what we just did for #8 and can re-use `BackendUserClient` + `AccountSyncOrchestrator`. The real Lambda+DynamoDB `BackendUserClient` (currently stubbed via `InMemoryBackendUserClient` in `WanderpastApp.init`) should land alongside #9/#10 — one-line swap, no other Swift code changes.
